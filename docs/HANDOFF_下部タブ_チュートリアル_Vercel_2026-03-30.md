...
- **選択タブの見た目（実機＝モック共通）**: **`globals.css`** で **`.tab-link[aria-current="page"]` と `.onboarding-mock-tab--active` を同一の `::before`（グロー）・`::after`（上ドット）**にまとめている。非選択タブの薄暗さは**どちらも付けない**。実機は `aria-current`、モックは **`onboarding-mock-tab--active`** クラス。`data-active` は React の表現差で使わない。
- **チュートリアル用モック**（`OnboardingDeviceMock`）: `onboarding-mock-tabbar` は **6 列 1 層**・**実機と同じデザイン仕様**。イントロで `activeHref === null` のときはどのセルも非選択表示（グロー・ドットなし）。
- **`OnboardingTutorial`**: 各ステップの **`screenshotSrc`（WebP）を削除**済み。常に `OnboardingDeviceMock` を表示（旧スクショに焼き込まれたタブだと「変更が反映されない」ように見えていた）。
- **オンボーディング完了キー**: `src/lib/onboarding-storage.ts` の **`maguro_note_onboarding_v6_done`**（内容更新のたびにキー名を上げる方針）。
...
- **キー**: `maguro_note_onboarding_v6_done` が `"1"` なら完了扱い。
